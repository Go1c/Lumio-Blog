...
describe('renderTagIndex empty state', () => {
  it('shows an empty state instead of the design demo tag cloud when there are no public tags', () => {
    const html = renderTagIndex(new Map(), config);

    expect(html).toContain('page-head');
    expect(html).toContain('暂无公开标签');
    expect(html).toContain('class="home-empty"');
    expect(html).not.toContain('渲染<span class="tag-pill__n">6</span>');
